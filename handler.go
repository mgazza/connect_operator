...
import (
	...
	"connect_operator/ksqlclient"
	"connect_operator/ksqlclient/swagger"
	samplev1alpha1 "connect_operator/pkg/apis/connect_operator/v1alpha1"
...
func (h *ConnectorHandler) Handle() error {
	...
	resp, err := h.kSQLClient.Describe(context.Background(), h.connection.Spec.Uri, managedResource.Spec.Name)
	if err != nil {
		msg := fmt.Sprintf(MessageExecutingKSQL, managedResource.Name, managedResource.Namespace, err)
